...
The Railway start command runs FastAPI on `127.0.0.1:8000` and Next.js on Railway's public port. Next.js rewrites `/api/python/*` to FastAPI. Public `/mcp` is an App Router proxy to FastAPI that forwards `Authorization` (rewrite alone drops Bearer / SSE and Cursor shows a red MCP light). The public path is `/mcp` with no trailing slash; the proxy must not return `Location: https://127.0.0.1:8000/mcp/`. Bearer auth is enforced by FastAPI (environment `MCP_API_KEY` **or** a key created in Dashboard → API KEY). Tool arguments are `{ node, request }`.
